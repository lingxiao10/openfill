...
export function modelPatch(body: Record<string, any>) {
	...
	// If the model name contains '/', it's routed through a proxy/router (e.g. OpenRouter uses
	// 'provider/model' format). The proxy handles format conversion itself, so we must NOT
	// convert tool_choice to Anthropic native format — the proxy expects OpenAI format.
	const isRoutedThroughProxy = model.includes('/')
	...
	if (modelName.startsWith('claude')) {
		...
		if (isRoutedThroughProxy) {
			// Proxy (OpenRouter etc.) translates OpenAI → Anthropic format itself.
			// Sending Anthropic-native tool_choice would confuse the proxy and result in
			// an empty tool name ("Tool '' not found in provided tools").
			// Leave tool_choice in OpenAI format; convert 'required' → { type: 'any' } only
			// if the proxy is known to pass it through (safe fallback).
			debug('Applying Claude patch (proxy mode): keeping tool_choice in OpenAI format')
		} else {
			// Direct Anthropic-compatible endpoint (LiteLLM, local proxy, etc.):
			// convert tool_choice to Anthropic native format.
