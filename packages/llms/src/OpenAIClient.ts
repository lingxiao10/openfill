...
export class OpenAIClient implements LLMClient {
	...
	async invoke(
		...
		if (!response.ok) {
			...
			// Some proxy providers (e.g. Anthropic via OpenRouter/proxy) wrap the real
			// error inside a JSON-string field called `raw`. Parse it if present.
			let parsedError: { error?: { message?: string; type?: string; code?: string } } =
				errorData ?? {}
			if (typeof (errorData as any)?.raw === 'string') {
				try {
					parsedError = JSON.parse((errorData as any).raw)
				} catch {
					// keep parsedError as errorData
				}
			}
		...
		switch (choice.finish_reason) {
			...
			case null: // OpenRouter and some proxies may return null
			case undefined: // defensive: treat missing as ok, validate tool call below
				break
			case 'length':
				throw new InvokeError(
