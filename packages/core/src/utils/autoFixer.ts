...
export function normalizeResponse(response: any, tools?: Map<string, PageAgentTool>): any {
	...
	if (toolCall?.function?.arguments) {
		...
		if (toolCall.function.name && toolCall.function.name !== 'AgentOutput') {
			...
			resolvedArguments = { actions: [safeJsonParse(resolvedArguments)] }
		}
	} else {
	...
	// normalize: convert legacy single 'action' to 'actions' array
	if (resolvedArguments.action && !resolvedArguments.actions) {
		log(`#compat: converting action to actions array`)
		resolvedArguments.actions = [safeJsonParse(resolvedArguments.action)]
		delete resolvedArguments.action
	} else if (resolvedArguments.actions) {
		// some models return actions as a non-array (single object / string) — wrap it
		if (!Array.isArray(resolvedArguments.actions)) {
			log(`#compat: wrapping non-array actions`)
			resolvedArguments.actions = [safeJsonParse(resolvedArguments.actions)]
		} else {
			resolvedArguments.actions = resolvedArguments.actions.map((a: any) => safeJsonParse(a))
		}
	}

	// validate and fix each action in the array using tool schemas
	if (resolvedArguments.actions && Array.isArray(resolvedArguments.actions) && tools) {
		resolvedArguments.actions = resolvedArguments.actions.map((a: any) => validateAction(a, tools))
	}

	// fix incomplete formats
	if (!resolvedArguments.actions || resolvedArguments.actions.length === 0) {
		log(`#5: fixing tool_call`)
		resolvedArguments.actions = [{ wait: { seconds: 1 } }]
	}
