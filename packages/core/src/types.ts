...
export interface AgentConfig extends LLMConfig {
	...
	/**
	 * Delay in seconds between sequential actions within a single step.
	 * Useful to let the page react before the next action is executed.
	 * @default 0.6
	 */
	actionDelay?: number
	 ...
	 * Sub-tasks always use the same model/baseURL/apiKey as the main agent.
	 */
	subTask?: {
		/**
		 * Maximum steps allowed per sub-task.
		 * @default 15
		 */
		maxSteps?: number
	}
...
export interface MacroToolResult {
	...
	outputs: { name: string; input: any; output: string; subHistory?: HistoricalEvent[] }[]
}
...
export interface AgentStepEvent {
	...
	actions: {
		...
		input: any
		output: string
		/** History from a run_subtask execution */
		subHistory?: HistoricalEvent[]
	}[]
	usage: {
		promptTokens: number
