...
import SYSTEM_PROMPT_ADDITIONS from './prompts/system_prompt_additions.md?raw'
import SYSTEM_PROMPT_SHARED from './prompts/system_prompt_shared.md?raw'
import SYSTEM_PROMPT_SUBTASK from './prompts/system_prompt_subtask.md?raw'
...
export type PageAgentCoreConfig = AgentConfig & {
	...
	/** @internal Marks this instance as a sub-task agent.
	 *  - Disables `run_subtask` tool (prevents infinite nesting)
	 *  - Strips subtask delegation rules from system prompt
	 *  - Skips mask show/hide (managed by the parent agent)
	 */
	_isSubTask?: boolean
}
...
export class PageAgentCore extends EventTarget {
	...
	constructor(config: PageAgentCoreConfig) {
		...
		if (this.config._isSubTask) {
			// Sub-task agents must not spawn further sub-tasks
			this.tools.delete('run_subtask')
			// Sub-task agents run autonomously — no human escalation
	...
	async execute(task: string, options?: { continueSession?: boolean }): Promise<ExecutionResult> {
		...
		// Show mask — skipped for sub-task agents (parent agent manages the mask)
		if (!this.config._isSubTask) {
		...
		while (true) {
			try {
				...
				const outputs = macroResult.outputs
				...
				// finish task if any action was 'done'

				const doneAction = outputs.find((o) => o.name === 'done')
				if (doneAction) {
					const success = doneAction.input?.success ?? false
					const text = doneAction.input?.text || 'no text provided'
	...
	#packMacroTool(): Tool<MacroToolInput, MacroToolResult> {
		...
		return {
			...
			execute: async (input: MacroToolInput): Promise<MacroToolResult> => {
				...
				const outputs: MacroToolResult['outputs'] = []
				const actionDelay = this.config.actionDelay ?? 0.6

				for (let i = 0; i < input.actions.length; i++) {
					// Inter-action delay (skip before the first action)
					if (i > 0 && actionDelay > 0) {
						await waitFor(actionDelay)
					}
					...
					const result = typeof rawResult === 'string' ? rawResult : (rawResult?.output ?? '')

					const subHistory = typeof rawResult === 'string' ? undefined : rawResult?.subHistory
					...
					outputs.push({ name: toolName, input: toolInput, output: result, subHistory })

					// Stop executing further actions after 'done'
	 ...
	 * - Main agent:     base prompt + shared + system_prompt_additions.md
	 * - Sub-task agent: base prompt (without subtask_rules) + shared + system_prompt_subtask.md
	 * - system_prompt_shared.md is always appended to both paths.
	 */
	#getSystemPrompt(): string {
		const stripComments = (md: string) =>
			md
				.split('\n')
		...
		const shared = stripComments(SYSTEM_PROMPT_SHARED)

		if (this.config.customSystemPrompt) {
			const additions = stripComments(SYSTEM_PROMPT_ADDITIONS)
			const parts = [this.config.customSystemPrompt, shared, additions].filter(Boolean)
			return parts.join('\n\n')
		}
		...
		if (this.config._isSubTask) {
			// Sub-task agents don't need the delegation rules (they ARE the delegate)
			systemPrompt = systemPrompt.replace(/<subtask_rules>[\s\S]*?<\/subtask_rules>\n*/g, '')
			const additions = stripComments(SYSTEM_PROMPT_SUBTASK)
			const parts = [systemPrompt, shared, additions].filter(Boolean)
			return parts.join('\n\n')
		}
	...
	async #assembleUserPrompt(): Promise<string> {
		...
		for (const event of this.history) {
			if (event.type === 'step') {
				...
				prompt += `Action Results: ${event.actions.map((a) => `${a.name}: ${a.output}`).join('\n')}\n`
				prompt += `</step_${stepIndex}>\n`
	...
	#onDone(success = true) {
		this.pageController.cleanUpHighlights()
		// Sub-task agents skip mask management (parent agent owns it)
		if (!this.config._isSubTask) {
			this.pageController.hideMask()
		}
