/**
 * Facebook 发帖脚本（文字 / 图片 / 视频）
 *
 * params:
 *   content    正文文字（可选，但纯媒体帖也可发）
 *   mediaPath  本地媒体绝对路径（可选，支持 jpg/png/gif/webp/mp4/mov/webm 等，由 facebook 单一 file input 接收）
 *   dryRun     'false' 才真实发布，默认 dry run
 *
 * 运行：cd packages/trainer && npx tsx src/tasks/facebook_post.ts
 */
...
console.log('[facebook] 页面就绪:', await page.getCurrentUrl())
...
ScriptStore.save(
	{
		id: 'facebook_post',
		name: 'Facebook 发帖（文字+图片+视频）',
		category: 'Facebook',
		description: '在 Facebook 主页发布一条帖子，支持纯文字、图片或视频',
		entryUrl: 'https://www.facebook.com/',
		params: [
			{ name: 'content', label: '正文', type: 'textarea', required: false },
			{ name: 'mediaPath', label: '媒体路径', type: 'text', required: false },
		],
		publishable: true,
	},
	`
...
console.log('[facebook] 脚本已保存到 ScriptStore')
console.log('[facebook] 脚本 ID: facebook_post')
