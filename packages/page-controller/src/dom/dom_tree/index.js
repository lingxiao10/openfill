     ...
     */
		...
		if (node.nodeType === Node.ELEMENT_NODE) {
			...
			if (nodeData.isVisible) {
				...
				// Special handling for ARIA containers and their items - check interactivity even if not top element.
				// This is needed when an inner element (e.g. a <span>) is the topmost element,
				// causing the interactive parent (e.g. <div role="option">) to be missed by isTopElement.
				const role = node.getAttribute('role')
				const BYPASS_TOP_ELEMENT_ROLES = new Set([
					// Container roles
					'menu', 'menubar', 'listbox', 'tree', 'grid', 'tablist',
					// Item roles — never topmost element due to inner text/icon children
					'option', 'menuitem', 'menuitemcheckbox', 'menuitemradio',
					'tab', 'treeitem', 'row', 'gridcell', 'columnheader', 'rowheader',
				])
				const isMenuContainer = role && BYPASS_TOP_ELEMENT_ROLES.has(role)
