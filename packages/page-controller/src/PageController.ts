...
import { POST_PARSE_DELAY, PRE_PARSE_DELAY } from './config'
...
export class PageController extends EventTarget {
	...
	async clickElement(index: number): Promise<ActionResult> {
		try {
			...
			// Snapshot indices before click so we can diff afterward
			const prevIndices = new Set(this.selectorMap.keys())

			await clickElement(element)

			// Wait before re-parsing to let the page start reacting
			await new Promise((r) => setTimeout(r, PRE_PARSE_DELAY * 1000))

			// Re-parse DOM and assign new element indices
			await this.updateTree()

			// Wait after re-parsing to let the page fully settle
			await new Promise((r) => setTimeout(r, POST_PARSE_DELAY * 1000))

			// Build change summary for AI context
			const changeSummary = this.buildClickChangeSummary(prevIndices)

			let message =
				element instanceof HTMLAnchorElement && element.target === '_blank'
					? `✅ Clicked element (${elemText ?? index}). ⚠️ Link opened in a new tab.`
					: `✅ Clicked element (${elemText ?? index}).`

			if (changeSummary) message += '\n' + changeSummary
	...
	/**
	 * Summarise DOM changes that occurred after a click.
	 * Result is appended to the action output so the AI sees what happened.
	 */
	private buildClickChangeSummary(prevIndices: Set<number>): string {
		const parts: string[] = []
		const currentIndices = new Set(this.selectorMap.keys())

		// 1. New interactive elements — the DOM parser already marks them with *[N]
		const newLines = this.simplifiedHTML
			.split('\n')
			.map((l) => l.trim())
			.filter((l) => /^\*\[\d+\]/.test(l))

		if (newLines.length > 0) {
			const preview = newLines.slice(0, 15).join('\n')
			const more = newLines.length > 15 ? `\n… and ${newLines.length - 15} more` : ''
			parts.push(`New elements:\n${preview}${more}`)
		}

		// 2. Disappeared interactive elements
		const disappeared = [...prevIndices].filter((i) => !currentIndices.has(i))
		if (disappeared.length > 0 && disappeared.length < 20) {
			parts.push(`Disappeared elements: [${disappeared.join(', ')}]`)
		}

		// 3. Changed-region HTML — only when the change is small (big = full navigation)
		const newCount = [...currentIndices].filter((i) => !prevIndices.has(i)).length
		if (newCount > 0 && newCount < 25) {
			const regions = this.findChangedRegions(prevIndices)
			if (regions.length > 0) {
				const htmlStr = regions
					.map((html, i) => `[Region ${i + 1}]:\n${html}`)
					.join('\n\n')
				parts.push(`Changed area HTML:\n${htmlStr}`)
			}
		}

		if (parts.length === 0) return ''
		return '--- Page changes after click ---\n' + parts.join('\n\n')
	}

	/**
	 * For each newly-appeared interactive element, find the closest meaningful
	 * container element and return its outerHTML (truncated).
	 */
	private findChangedRegions(prevIndices: Set<number>): string[] {
		const STOP_TAGS = new Set([
			'dialog', 'aside', 'section', 'nav', 'ul', 'ol',
			'form', 'header', 'footer', 'main', 'table', 'tbody', 'thead',
		])
		const MAX_HTML = 2000
		const MAX_REGIONS = 3
	...
	/**
	 * Click any element by CSS selector, including non-indexed blank areas.
	 */
	async clickBlankArea(selector: string): Promise<ActionResult> {
		try {
			const message = await clickElementBySelector(selector)
			return { success: true, message }
		} catch (error) {
			return { success: false, message: `❌ Failed to click blank area: ${error}` }
		}
	}

	/**
	 * Type text into input and immediately press Enter (for tag/chip inputs).
	 */
	async inputTextAndEnter(index: number, text: string): Promise<ActionResult> {
		try {
			this.assertIndexed()
			const element = getElementByIndex(this.selectorMap, index)
			const message = await inputTextAndEnterElement(element, text)
			return { success: true, message }
		} catch (error) {
			return { success: false, message: `❌ Failed to input text and enter: ${error}` }
		}
	}

	/**
	 * Focus element by index without typing.
	 * Used by debugger mode: background CDP input requires the element to be focused first.
	 */
	async focusElement(index: number): Promise<ActionResult> {
		try {
			this.assertIndexed()
			const element = getElementByIndex(this.selectorMap, index)
			await clickElement(element)
			...
			return { success: false, message: `❌ Failed to focus element: ${error}` }
		}
	}
	...
	/**
	 * Send a keyboard key to an element by index, or to the currently active element.
	 */
	async sendKeys(key: string, index?: number): Promise<ActionResult> {
		try {
			...
			const message = await sendKeysToElement(element, key)
			return { success: true, message }
		} catch (error) {
			return { success: false, message: `❌ Failed to send key: ${error}` }
		}
	}
