...
export function useAgent(): UseAgentResult {
	...
	const loggedHistoryLenRef = useRef(0)
	...
	useEffect(() => {
		...
		const handleHistoryChange = (e: Event) => {
			...
			// Log any newly added events
			for (let i = loggedHistoryLenRef.current; i < newHistory.length; i++) {
				debugLogEvent(newHistory[i])
			}
			loggedHistoryLenRef.current = newHistory.length
			setHistory([...newHistory])
		}

		const handleActivity = (e: Event) => {
			const newActivity = (e as CustomEvent).detail as AgentActivity
			debugLogActivity(newActivity)
