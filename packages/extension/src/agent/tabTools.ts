...
export function createTabTools(tabsController: TabsController): Record<string, TabTool> {
	return {
		...
		reload_page: {
			description: 'Reload (refresh) the current tab. Waits until the page is fully loaded.',
			inputSchema: z.object({}),
			execute: async (_input: unknown) => {
				const tabId = tabsController.currentTabId
				if (tabId == null) return '❌ Failed: No current tab.'
				try {
					return await tabsController.reloadTab(tabId)
				} catch (error) {
					return `❌ Failed: ${error instanceof Error ? error.message : String(error)}`
				}
			},
