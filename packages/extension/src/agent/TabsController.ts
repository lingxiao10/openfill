...
export class TabsController extends EventTarget {
	...
	async init(task: string, includeInitialTab: boolean = true) {
		...
		const tabChangeHandler = (message: any): void => {
			...
			if (message.action === 'created') {
				...
				const isInOurGroup = this.tabGroupId !== null && tab.groupId === this.tabGroupId
				// Also track tabs opened by our tracked tabs (e.g. target=_blank clicks)
				const isOpenedByOurTab =
					tab.openerTabId != null && this.tabs.some((t) => t.id === tab.openerTabId)
				if ((isInOurGroup || isOpenedByOurTab) && tab.id != null) {
					// Tab created in our controlled group or opened by our tab
	...
	async reloadTab(tabId: number): Promise<string> {
		...
		if (result.success) {
			...
			throw new Error(`Failed to reload tab ID ${tabId}: ${result.error}`)
		}
	}
